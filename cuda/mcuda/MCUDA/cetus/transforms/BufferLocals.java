 ...
 * need to be buffered.
 */
public class BufferLocals extends KernelTransformPass
{
  ...
  public BufferLocals(Program program)
  {
    super(program);
  }

  public void MakeArray(VariableDeclaration vDecl, Traversable root, Procedure proc)
  {
    ...
    { 
      //Move the buffered array outside of the procedure scope.
      TranslationUnit file = (TranslationUnit) proc.getParent();

      DeclarationStatement stmt = (DeclarationStatement) vDecl.getParent();
      
      root.removeChild(stmt);
      
      file.addDeclarationBefore(proc, vDecl);
    }
	...
	variableID = (Identifier)iter.next(Identifier.class);
      ...
      //If we've just found a declaration, skip it 
...
//	else
  	...
  	if(MCUDAUtils.getThreadLoopBody(variableID) == null)
	{
	  ...
	  {
	    Tools.printlnStatus("Error: " + variableID.toString(), 3);
	  }
	  */
	  variableID.swapWith((Expression)variableStoreZero.clone());
	  continue;
  ...
  public void transformProcedure(Procedure proc)
  {
	...
	continue;
      ...
      //Never need to buffer a const variable with a thread-independent 
      //value
      if(((vDecl.getSpecifiers()).contains(Specifier.CONST)))
      {
	if(vDecl.getDeclarator(0).getInitializer() == null)
	  continue;
      }
	...
	while(iter2.hasNext())
	{
      ...
      {
	MakeArray(vDecl, vDecl.getParent().getParent(), proc);
      }
