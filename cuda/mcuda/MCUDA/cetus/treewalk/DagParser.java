...
public class DagParser
{
  ...
  class NodeBase
  {
    ...
    public NodeBase(int node_id, String node_info)
    {
      this.node_id = node_id;
      this.node_info = new String(node_info);
    }
  };
  ...
  private GeneralSyntaxNode fetchNode(NodeBase target)
  {
    GeneralSyntaxNode  _node = (GeneralSyntaxNode)(node_table.remove(new Integer(target.node_id)));
    if (_node == null)  
    {
      //this is a leaf node, just creat one leaf node and return
      GeneralSyntaxNode  leaf_node = new GeneralSyntaxNode(target.node_id, null, target.node_info);
      return leaf_node;
    }
    else
    {
  ...
  protected GeneralSyntaxNode createNode(NodeBase node)
  {
    GeneralSyntaxNode  new_node = new GeneralSyntaxNode(node.node_id, null, node.node_info);
    return new_node;
  }

  private void hashSyntaxNode(int node_id, GeneralSyntaxNode node_to_hash)
  {
  ...
  public GeneralSyntaxNode run(String dag_file_name)
  {
    BufferedReader dag_file = null;
    GeneralSyntaxNode lhs_syntax_node = null;
    ...
    while (true)
    {
      ...
      else if (next.equals("->"))
      {
        ...
        while (!(s = input_buffer_monitor.get()).equals("}"))
        {
          ...
          node_base  = new NodeBase(node_id, node_info);

          GeneralSyntaxNode child_syntax_node = fetchNode(node_base);
