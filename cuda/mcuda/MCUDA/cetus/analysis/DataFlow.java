...
public class DataFlow
{
  ...
  private static void addDef(TreeMap<Expression, Expression> var_to_guard, Expression defined, Expression guard)
  {
    if (var_to_guard.containsKey(defined))
    {
      ...
      {
        var_to_guard.put(defined, new BinaryExpression((Expression)existing_guard.clone(),
          BinaryOperator.LOGICAL_OR, (Expression)guard.clone()));
      }
    }
    else
      var_to_guard.put(defined, guard);
    ...
    Expression guard, Traversable root)
  {
    ...
    else if (root instanceof IfStatement)
    {
      ...
      Expression condition = (Expression)if_stmt.getControlExpression().clone();
      if (guard != null)
      {
        condition = new BinaryExpression((Expression)guard.clone(), BinaryOperator.LOGICAL_AND,
      ...
      if (if_stmt.getElseStatement() != null)
      {
        Expression inverted_condition = new UnaryExpression(UnaryOperator.LOGICAL_NEGATION,
          (Expression)if_stmt.getControlExpression().clone());

        if (guard != null)
        {
          inverted_condition = new BinaryExpression((Expression)guard.clone(), BinaryOperator.LOGICAL_AND,
                                           (Expression)inverted_condition.clone());
        }
    ...
    {
      Loop loop = (Loop)root;
      Expression condition = (Expression)loop.getCondition().clone();
