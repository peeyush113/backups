...
#define POP_SIZE                  100			//must be an even number ... WHY ???
#define CHROMO_LENGTH             300
#define GENE_LENGTH               4
#define MAX_ALLOWABLE_GENERATIONS	400

//returns a float between 0 & 1
#define RANDOM_NUM		(rand()%2)
...
int main()
{
  ...
  while (true)
  {
    ...
    cout << endl << endl;
	  ...
	  //---->>>CAN BE PARALLELIZED
	  //---->>>HOW : each thread generates a random number on it's own and process a unique population[i].
	  for (int i=0; i<POP_SIZE; i++)
	  {
		  Population[i].bits	  = GetRandomBits(CHROMO_LENGTH); //Poppulation[i].bits contain string of length 300 with random 0s and 1s
      ...
      // population
	  //---->>>CAN BE PARALLELIZED
	  //---->>>HOW : each thread process a unique population[i].
		  for (int i=0; i<POP_SIZE; i++)
		  {
		  ...
		  //---->>>CAN BE MERGED WITH THE ABOVE LOOP!!
		  for (int i=0; i<POP_SIZE; i++)
		  {
